...
function processImportRow(sourceFile, lineNumber, _values, valueByStandardName, rawLine) {
    ...
    if ((valueByStandardName['AMOUNT GBP'] ?? '').trim()) {
        ...
        if (amtNorm === null)
            reasons.push('Montant (AMOUNT GBP) non numérique');
        else
            row['AMOUNT GBP'] = amtNorm;
    }
    else {
    ...
    if (row.AMOUNT.trim() && amountNorm === null)
        reasons.push('AMOUNT non numérique');
    else if (amountNorm !== null)
        row.AMOUNT = amountNorm;
    const currencyNorm = normalizeAmount(row.CURRENCY);
    if (row.CURRENCY.trim() && currencyNorm === null)
        reasons.push('CURRENCY non numérique');
    else if (currencyNorm !== null)
        row.CURRENCY = currencyNorm;
...
async function readExistingProcessed(appPath) {
    const fullPath = path.join(appPath, dataPaths_1.SOURCE_DATA_PATH);
...
async function mergeImportTransactions(appPath) {
    ...
    const processedFull = path.join(appPath, dataPaths_1.SOURCE_DATA_PATH);
    ...
    if (csvFiles.length === 0) {
        ...
        const processedDir = path.join(appPath, path.dirname(dataPaths_1.MERGE_REPORT_PATH));
        if (!(0, fs_1.existsSync)(processedDir))
            await fs.mkdir(processedDir, { recursive: true });
        const reportFull = path.join(appPath, dataPaths_1.MERGE_REPORT_PATH);
    ...
    return {
        ...
        reportPath: dataPaths_1.MERGE_REPORT_PATH,
    };
}
async function getLastImportReportPath(appPath) {
    ...
    if (!(0, fs_1.existsSync)(reportFull))
        return null;
    return dataPaths_1.MERGE_REPORT_PATH;
}
